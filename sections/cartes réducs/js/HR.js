...
window.addEventListener("load", () =>
	Promise.all([
		getImg("./assets/BF2.png").then((img) => (passeport = img)),
...
const canvadraw = async (
	...
	ctx.fillText("MENSUEL", 201 * MULTIPLIER, (90 + 15) * MULTIPLIER);
	ctx.fillText(passportNum, 388 * MULTIPLIER, (90 + 15) * MULTIPLIER);
...
const SubmitIDForm = () => {
	...
	const IDCardData = {
		...
		ID_Picture: Picture,
		ID_BirthPlace: clear(form.children["BirthPlace"].value, MAX_NAMES_LENGTH),
	};
	...
	if (
		[
			...
			errorLabel(!["M", "F", "X", "Taupe"].includes(IDCardData.ID_Sex), "Sex"),
		].some((e) => e === true)
	)
		return;
...
const ApplyIDCard = async (IDCardData) => {
	...
	await canvadraw(
		...
		clear(IDCardData.ID_BirthPlace, MAX_NAMES_LENGTH),
