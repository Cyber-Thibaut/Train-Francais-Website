    ...
    if (isVacation === true) {
        if (weekday === 6) { // Samedi
            ...
            if (hour >= 4 && hour < 7) {
                ...
                followingFrequency = 24; // Pointe
            } else if (hour >= 9 && hour < 16) {
                followingFrequency = 18; // Creuse
            } else if (hour >= 16 && hour < 20) {
                followingFrequency = 24; // Pointe
            } else {
                followingFrequency = 36; // Soir
            }
        } else if (weekday === 0) { // Dimanche
            if ((hour === 1 && minute >= 0) || (hour >= 2 && hour < 4)) {
                return -1; // Créneau fermé
            ...
            // Fréquences pour Semaine
            if (hour >= 4 && hour < 7) {
                followingFrequency = 24; // Creuse
            } else if (hour >= 7 && hour < 9) {
                followingFrequency = 18; // Pointe
            } else if (hour >= 9 && hour < 16) {
                followingFrequency = 24; // Creuse
            } else if (hour >= 16 && hour < 20) {
                followingFrequency = 18; // Pointe
            } else {
                followingFrequency = 72; // Soir
            }
        }
    } else {
        if (weekday === 6) { // Samedi
...
function updateBusTimes() {
    ...
    if (nextBusFreq === -1) {
        ...
        const nextBus = nextBusFreq === 1 ? `<span style='color: #007229;'>A quai  ${sign} ${frequentation1}</span>` : nextBusFreq === 2 ? `<span style='color: #007229;'>A l’approche ${sign} ${frequentation1}</span>` : `<span style='font-size: 40px; color: #007229;'>${nextBusFreq} min ${sign} ${frequentation1}</span>`;

        const followingBus = followingBusFreq === 0 ? `<span style='color: #007229;'>Arrivé  ${sign}</span>` : followingBusFreq === 1 ? `<span style='color: #007229;'>1 min ${sign}</span>` : `<span style='font-size: 40px; color: #007229;'>${followingBusFreq} min ${sign} ${frequentation2}</span>`;
        document.getElementById("nextBusTime").innerHTML = nextBus + followingBus;
    }
